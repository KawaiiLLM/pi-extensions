...
export function createStartupCheck(
	loaders: SyncLoaders,
	...
	return {
		start(ctx: ExtensionContext, sessionSignal: AbortSignal, initialConfig?: AnySyncConfig) {
			...
			active = task;
			const isCurrent = () =>
				!sessionSignal.aborted && active === task && task.owner === ctx.sessionManager;
				...
				.then(async () => {
					try {
						...
						if (config) {
							...
							if (
								!isCurrent() ||
								(signal.aborted && controller.signal.reason?.name !== "TimeoutError")
							)
								return;
							if (
								!latest ||
