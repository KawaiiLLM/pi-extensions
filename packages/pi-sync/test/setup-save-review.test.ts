...
const lines = [
	...
	...Array.from(
		{ length: 30 },
		(_, index) => `Included path ${index}: exact/${index}/settings.json`,
	),
	"Automatic sync: Off",
...
test("a retryable save failure retains the same draft and retries only on confirmation", async () => {
	...
	const pending = saveReviewedDraft(
		ctx,
		"Review sync setup",
		["Exact path: work/archive"],
		"Save setup",
		async () => {
			if (++attempts === 1)
				throw Object.assign(new Error("Cannot write settings"), { code: "ENOSPC" });
		},
	);
	await tui.waitForOpen();
	tui.press("tui.select.confirm");
	...
	async (action) => {
		...
		const keybindings = {
			...
			matches: (data: string, binding: string) =>
				keybindings.getKeys(binding).some((key) => matchesKey(data, key)),
		};
