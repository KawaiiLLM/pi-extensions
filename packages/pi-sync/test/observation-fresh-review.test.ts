		...
		await withTempHome(async (agentDir) => {
			...
			try {
				...
				assert.equal(
					...
					superseded
						? undefined
						: remote === "replacement" || remote === "new-observation"
							? replacement
							: observation,
				);
				if (superseded) {
					...
					assert.doesNotMatch(
						managerFrames[1] ?? "",
						/Review synced content|Synced-content list differs/u,
					);
				...
				if (remote === "legacy")
					assert.match(
						context.notifications.map((n) => n.message).join("\n") + frames.join("\n"),
						/legacy|partial/u,
					);
			} finally {
				factory.mockRestore();
				...
				await backend.publishSnapshot(
					{
						...
						...(remote === "legacy"
							? {}
							: { selection: { version: 1 as const, include: config.include } }),
					},
					{ kind: "missing" },
				);
			let resolved = 0;
