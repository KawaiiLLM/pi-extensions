...
test.each(cases)(
	...
	async ({ status, body, outcome, message }) => {
		...
		const output = (await new S3SyncBackend(config).diagnose())
...
test.each([false, true])(
	...
	async (declaredLength) => {
		...
		const output = (await new S3SyncBackend(r2).diagnose())
			.map((entry) => entry.message)
			.join("\n");
...
test.each(["transport", "timeout", "cancel"])(
	"S3 diagnostic handles $0 without writes",
	async (kind) => {
