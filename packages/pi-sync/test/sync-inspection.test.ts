	...
	test(`inspection: ${scenario.name} remains advisory and read-only`, async () => {
		await withTempHome(async (agentDir) => {
			...
			await writeFile(
				localConfigPath(),
				JSON.stringify(
					v3S3Settings({ include: scenario.include ?? ["settings.json", "AGENTS.md"] }),
				),
			);
			const config = await loadConfig();
			if (!scenario.missingLocal)
				await writeFile(path.join(agentDir, "settings.json"), scenario.local ?? "baseline");
			if (!scenario.first)
				await writeStateForConfig(config, {
					...
					lastFileHashes: {
						"settings.json":
							snapshot([{ path: "settings.json", content: Buffer.from("baseline") }]).files[0]
								?.sha256 ?? "",
					},
			...
			assert.deepEqual(
				await readFile(statePathForConfig(config)).catch(() => undefined),
				stateBefore,
			);
			if (!scenario.missingLocal)
				assert.equal(
...
test("inspection honors custom session roots and protects excluded credential files", async () => {
	await withTempHome(async (agentDir) => {
		...
		await writeFile(
			localConfigPath(),
			JSON.stringify(v3S3Settings({ include: ["sessions", "custom"] })),
		);
		const config = await loadConfig();
		const files = [
		...
		const result = await inspectSync(
			config,
			{ include: config.include, sessionDir },
			undefined,
			async () => backend,
		);
