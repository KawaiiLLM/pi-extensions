...
- Report cost for today rather than for the session: `☉ $186.00 (93%)` is the current provider's spend since local midnight, read from every session on disk, beside today's share of a daily budget. The budget is a seventh of the weekly window, computed in percent of the window so it survives a window nobody has used yet; the segment inverts its colors at 100%.
